...
export function Tile({ label, value, badge, hint = "Tap pour modifier", onClick }: TileProps) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex w-full flex-col justify-between rounded-2xl border border-border bg-background p-3 text-left cursor-pointer",
        "shadow-sm transition active:scale-[0.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
      )}
    >
